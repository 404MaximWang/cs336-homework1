...
def run_rope(
    ...
    """
    return rope(d_k, theta, in_query_or_key, token_positions)
...
def run_get_lr_cosine_schedule(
    ...
    """
    return get_lr_cosine_schedule(it, warmup_iters, min_learning_rate, max_learning_rate, cosine_cycle_iters)
...
def get_tokenizer(
    ...
    """
    vocab_inverted: dict[bytes, int] = {v: k for k, v in vocab.items()}
    merges_adapted: dict[tuple[int, int], int] = {}
    for b1, b2 in merges:
        merges_adapted[vocab_inverted[b1], vocab_inverted[b2]] = vocab_inverted[b1 + b2]
    return Tokenizer(vocab = vocab, merges = merges_adapted, special_tokens = special_tokens)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    max_bytes = kwargs.get('max_train_bytes', None)
    tokenizer.train(input_path, vocab_size, max_bytes = max_bytes)
    merges_byte: list[tuple[bytes, bytes]] = []
    for merge in tokenizer.merges:
        merges_byte.append((tokenizer.vocab[merge[0]], tokenizer.vocab[merge[1]]))
    return tokenizer.vocab, merges_byte